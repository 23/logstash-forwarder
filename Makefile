...
generate-init-script:
	pleaserun --install --no-install-actions --install-prefix ./build \
		...
		--overwrite -p sysv -v lsb-3.1 $(PREFIX)/bin/logstash-forwarder -quiet -config /etc/logstash-forwarder/conf.d
...
rpm deb: compile generate-init-script build/empty
	fpm -f -s dir -t $@ -n logstash-forwarder -v $(VERSION) \
		...
		./build/etc=/ \
		./build/empty/=/etc/logstash-forwarder \
		./build/empty/=/etc/logstash-forwarder/conf.d \
